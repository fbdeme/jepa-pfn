...
from ._base import PriorComponent, Dataset, DatasetProperties, FeatureSpec
from ._graph import RandomDAG
from ._graph_function import RandomGraphFunction
...
class RandomDataset(PriorComponent):
    def sample(self, data_prop: DatasetProperties) -> Dataset:
        while True:
            ...
            feature_groups = sorted(set(spec.group for spec in data_prop.feature_specs.values()))   # jepa-pfn: list(set(str)) order depends on PYTHONHASHSEED -> stream differed across processes
